...
' [x; y];
:= [N; 24; SIZE; 360; rec; 0; PLAY_ICON; "⏵"; PAUSE_ICON; "⏸"; ALIVE_COLOR; "#f12"; DEAD_COLOR; "#222"; CELL_WIDTH; * [SIZE; / [N]]; CELL_HEIGHT; * [SIZE; / [N]];
    r; N; h; * [N; -1]; bounds; * [N; N]; init; -> [N; |> [.: ... [N]; .: map >> [-> [0]]]]; cells; init [bounds]; next; init [bounds];
    directions; .: [
  :: [x; 0; y; 1];
  :: [x; 1; y; 0];
  :: [x; -1; y; 0];
...
:= [ctx; |> [canvas []; 
    ...
    click [-> [e; : [
              := [X; math_floor [* [::. [e; "ox"]; / [CELL_WIDTH]]]];
              := [Y; math_floor [* [::. [e; "oy"]; / [CELL_HEIGHT]]]];
...
|> [:= [control; element ["bt"]]; text [PLAY_ICON]; click [-> [: [text [control; ? [= [rec; ![rec]]; PAUSE_ICON; PLAY_ICON]]; play[]]]]; 
    attach [root]];