...
import { Expression, Interpretion } from './index.js'
import { extensions } from '../extensions/extensions.js'

export const VOID = 0
export const extract = (item: Expression, env: Expression) =>
  item.type === 'value' ? item.value : evaluate(item, env)
const tokens: Record<string, Interpretion> = {
  ['+']: (args, env) => {
    if (args.length < 2)
  ...
  ['assert:']: (args, env) => {
    ...
    if (!evaluate(args[0], env))
      throw new Error(
        `${args[2] ? evaluate(args[2], env) : ''} ${evaluate(args[1], env)}`
      )
  },
  ['*loop']: (args, env) => {
    if (args.length !== 2)
  ...
  ['aliases=']: (args, env) => {
    ...
    for (let i = 0; i < args.length; ++i) {
      if (i % 2 === 0) {
        ...
        if (word.type !== 'word')
          throw new SyntaxError(
            `First argument of aliases= [] must be word but got ${
              word.type ?? VOID
            }`
          )
        if (word.name in tokens)
